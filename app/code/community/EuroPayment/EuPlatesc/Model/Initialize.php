...
class EuroPayment_EuPlatesc_Model_Initialize extends Mage_Payment_Model_Method_Abstract 
{
	...
	public function getCheckoutFormFields()
	{
		...
		*/
		...
		$orderIncrementId = $this->getCheckout()->getLastRealOrderId() ? $this->getCheckout()->getLastRealOrderId() : Mage::getSingleton('customer/session')->getLastIncrementId();
        $order = Mage::getModel('sales/order')->loadByIncrementId($orderIncrementId);
        Mage::log('increment id from session (Initialize.php euPlatest Gateway) ' . Mage::getSingleton('customer/session')->getLastIncrementId(), false, 'euPlatesc.log');
		...
		foreach ($items as $item) {
			$items_to_string = 
			      'Denumire: ' . $this->clean4euplatesc($item->getName()) . 
			      ' (Cod: ' . $this->clean4euplatesc($item->getSku()). 
			      ') Cantitate: ' . number_format($item->getQtyOrdered(), 0, '.', '') . 
			      ' / Valoare: ' . Mage::helper('core')->currency($item->getRowTotalInclTax() - $item->getDiscountAmount(), true, false) . 
                  '<br />'; // customized by np
		}
        
		$customer = $this->getCustomer();
        $address = $order->getIsVirtual() ? $order->getBillingAddress() : $order->ShippingAddress(); // customized by np
        $amount = $order->getBaseGrandTotal();

		$billing	= $order->getBillingAddress();
		$billingId	= (int)$billing->getCustomerAddressId();
		
		$shipping = $order->getIsVirtual() ? $order->getBillingAddress() : $order->ShippingAddress(); // customized by np
		...
		$sArr = array(
			...
			'order_desc'	=> "Comanda online  " . Mage::app()->getStore()->getFrontendName() . "<br />" . $items_to_string, // customized by np
