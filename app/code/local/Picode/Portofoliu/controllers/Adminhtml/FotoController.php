...
class Picode_Portofoliu_Adminhtml_FotoController extends Mage_Adminhtml_Controller_Action {

	protected function _initAction()
	{
		$this->loadLayout()
			->_setActiveMenu('foto/items')
			->_addBreadcrumb(Mage::helper('adminhtml')
			->__('Portofolii Foto'), Mage::helper('adminhtml')
			->__('Portofolii Foto'));
	...
	public function editAction()
	{
		...
		if ($model->getId() || $id == 0) {
			...
			$this->loadLayout();
			$this->_setActiveMenu('foto/items');
			 ...
			 * adminhtml/portofoliu/edit.phtml
			 * adminhtml/portofoliu/edit/tabs.phtml
			 * so, I commented out the next line
			*/
...
// 
	...
	public function deleteAction()
	{
		...
		if ($this->getRequest()->getParam('id') > 0) {
			try {
				$model = Mage::getModel('portofoliu/albums');
	...
	{
		$portofoliuIds = $this->getRequest()->getParam('albums');
		// Zend_Debug::dump($portofoliuIds); die();
		...
		if (!is_array($portofoliuIds)) {
			...
			try {
				foreach ($portofoliuIds as $portofoliuId) {
					$portofoliu = Mage::getModel('portofoliu/albums')->load($portofoliuId);
					// Zend_Debug::dump($portofoliu); die();
					$portofoliu->delete();
				}
